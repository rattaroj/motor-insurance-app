...
public class GetPolicyDocumentEndpoint : EndpointWithoutRequest
{
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        var p = await _db.Policies.AsNoTracking()
            ...
            .Select(x => new
            {
                x.PolicyNo, x.Status, x.CoverageType, x.SumInsured, x.BasePremium,
                x.NcbPercent, x.Deductible, x.EffectiveDate, x.ExpiryDate,
                x.VehicleId, x.QuotationId, x.PreviousPolicyId,
                CustomerName = x.Customer.FullName, x.Customer.NationalId, x.Customer.Phone, x.Customer.Email,
                Reg = x.Vehicle.RegistrationNo, Prov = x.Vehicle.Province, Chassis = x.Vehicle.ChassisNo,
